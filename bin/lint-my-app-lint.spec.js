...
describe('lint-my-app lint', () => {
	...
	it('continues after an error', async() => {
		// FIXME Where are all the unhandled exceptions coming from?

		execa.mockImplementation((command) => {
			const promise = command === 'pkg-ok' ?
				Promise.reject({ stdout: 'stdout', stderr: 'stderr' }) : // eslint-disable-line prefer-promise-reject-errors
				Promise.resolve();
			promise.stdin = jest.fn();
			promise.stdout = { pipe: jest.fn() };
			return promise;
		...
		await expect(lint()).rejects.toThrow('Something went wrong');
